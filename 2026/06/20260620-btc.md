...
## まだおわっていない(2026/06/29)

結局、異音がしていたHDDをまだ使ってBitcoin Coreの同期中である。  
まだ終わっていない。。。ようやく2021年になったくらいだ。

一度、止まったのだ。  
`reindex-chainstate`でやっていたのだが、なんかそれではダメそうな雰囲気。
かといってディスクがだめになったわけではなさそう。  
これはRaspberry Piでディスクがおかしいときのことが原因ではなかろうかとあきらめて`reindex`の方でやり直しているのだ。

思い起こせば、結構な頻度で`reindex`している。
半年に一度くらいだろうか。

* [rpi: the root account is locked - hiro99ma blog](https://blog.hirokuma.work/2025/11/20251102-linux.html)
* [rpi: HDDの調子がよくないのでツールにチェックしてもらう - hiro99ma blog](https://blog.hirokuma.work/2026/02/20260210-linux.html)

2026/02/10のあとにその2.5インチのHDDは破棄した。
今あるのは残った1台の方である。

HDDだと時間がかかりすぎるので余ったSSDを使って同期して、そのデータをHDDにコピーするということをやっていた。
それがこのときだったのかどうか覚えていない。  
ただ、データは消さずに次回のために残すことにしたのは覚えていた。  
ノートPCもマウントポイントは残したままだったので、挿すとすぐに使用できた。  
そしてBitcoin Coreが指す先を変更して開始・・・したのだがディスクの調子が悪いようなことを言い始めた。  
こいつもか！

そもそも余ったSSDというのは使用率が100%(空き容量じゃなくてね)になったので使うのを止めていたのだ。
なにかおかしくても不思議ではない。  
しかしWindowsでMiniToolをインストールしてサーフェステストをしたのだが問題なかった。  
もしかすると、数カ月使わなかったことで電気的に抜けたりした？  
記事を読んだときは1年くらい未使用だとそういうこともある、という感じだったけど、まさか。。。

そんな感じで寄り道しながら同期中です。
今週中に終わればいいなあ。
`reindex`しなくてよかったらもっとあっさりと終わっていたはずだったのだが、世の中うまく行かないことばかりよ。
