...
### ローカル実装のブランケット実装を確認する

`cargo doc` でdocs.rs を作るのが簡単らしい。
出来上がった `target/doc/<クレート名>/index.html` をブラウザで開くとよい。

```bash
cargo doc
```

ブランケット実装として `ToString` が載っている。
`Display` は自分で実装しているから "Trait Implementations" に載っているということか。

![image](images/20260425a-3.png)

### 標準のブランケット実装

初めて docs を作ったけど、覚えがない `Any` や `Borrow` などがあるし、
"Auto Trait Implementations" にもいろいろ載っているし、何なんだろう？  
`Display` 実装を削ってみたが変わらない。

どうやら標準で追加されるブランケット実装というものがあるらしい。

* `Auto`
* `Borrow<T>`
* `From<T> for T`
  * `Into<U> for U where U: From`

`Into` は `From` のブランケット実装らしい。

### Auto Trait Implementationsは別

サイドバーに "Auto Trait Implementations" という項目がある。
自動で実装されるってことならブランケット実装と同じものじゃないかと思ったが、そうでもないらしい。

特定のトレイト実装があることで自動的に追加されるのがブランケット実装、
コードの中身によって自動的に追加されるのが Auto Trait Implementations だそうだ。
この例だと、`Send` や `Sync` はフィールドが `i32` だから大丈夫だから付けておこう、という理屈らしい。

どちらも自動的に追加されるものではあるが紛らわしい気が。。。いや、私よりも命名が上手な人たちで考えたのだろうから止めておこう。

## 慣れの問題なのか

トレイトまではまだよいのだが、ブランケット実装や他のトレイトに関する技のような実装を見ていると、
そこまでしなくても明示的に実装したほうがわかりやすいんじゃないかという気持ちになってしまった。

`From` と `Into` があって、ジェネリックな引数で `Into<T>` を取って中で `.into()` を呼ぶことで `From` が呼ばれてうまくいく、
とかもう想像がつかないというかなんというか。  
慣れの問題なんだろうか。  
まあ、使わないと実装できないというわけではないのが救いか。
