...
[exit](https://doc.rust-lang.org/std/process/fn.exit.html) も `fn main() -> Result<(), MyError>` にしているし、学習が進むまではそういうものだと思っておけば良いのかな。
...
Export したのに。。。  
Sparrow Wallet でも Electrum Wallet でも Export では xpub しかできないようだった(コマンドライン操作でできたのかも？)。  
