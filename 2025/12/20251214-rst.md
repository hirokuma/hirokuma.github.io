...
`tracing_log::LogTracer` を挟むことで `log::*` をディスパッチするようなしくみはあるそうだ。
ライブラリだとどうしようか悩むかもしれないけど、全部自分のアプリでしか使わないとなると `use tracing::*` でいいんじゃないのと思ってしまう。  
ちょっと悩もう。
...
### 最近は tracing::subscriber::set_global_default()？

最初に見た `tracing::subscriber` を使った設定が `.init()` だったのでそうしていた。  
Gemini 氏が「最近は `tracing::subscriber::set_global_default()` ですよ」と言ってきた。

それくらいよいかと置き換えると、自作コードの `log::*` のところだけログが出なくなった。  
そしたら「`LogTracer::init()` がいるよ」と言い出した。  
確かに出力されるようになったけど、それなら `.init()` でよかったやんと思った。

[README](https://github.com/tokio-rs/tracing/blob/ee82cf92a8c750f98cfb7a417cc8defb37e26a00/README.md#in-applications) も `init()` だし。
`init()` は `set_global_default()` を呼び出すとも書いてあるんで、もうそれでいいんじゃないだろうか。  

DeepWiki によると `tracing_subscriber::Registry::default().init()` は内部で `LogTracer::init()` を呼んでいるそうだ。  
なので `LogTracer::init()` がなくてもよいはず。

* [DeepWiki](https://deepwiki.com/search/tracingsubscriber-logtrace_1293ac6d-b902-4520-86d0-9e8895495576?mode=fast)
