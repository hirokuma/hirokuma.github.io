...
だがこの方法、Cargo.toml と同じパスに置けばよいのかと思ったが `PATH` が通った場所に置くという意味だった。。。
それだと `core.hooksPath` とあまり変わらないな。`PATH` を通せば良いだけだから汎用性はちょっとあるかもしれないが、
`.githooks/pre-push` にスクリプトを書いて `git config --local core.hooksPath "./githooks"` とした方がわかりやすいように思った。cargo 関係じゃなくて git 関係だしね。
