...
`thiserror` を使って自分のエラー型を作ったのだから、関数の戻りエラーもそうするのだ。  
`anyhow::Result` ではないので `main()` 側の実装も変わってくる。今回は省略したが ChatGPT のサンプルコードがあるからよいだろう。

```rust
fn f() -> Result<(), AppError> {
    Err(AppError::Io(io::Error::new(io::ErrorKind::NotFound, "oh no!")))
}
```

`thiserror` 版と `anyhow` 版、みたいに思うと良いか。  
