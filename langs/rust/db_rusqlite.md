...
#### query_mapの最初の`[]`はなんなのよ

SELECTの結果を読み出すのに[query_map](https://docs.rs/rusqlite/latest/rusqlite/struct.Statement.html#method.query_map)を使っていたが、
サンプルコードでは第1引数に`[]`を使っていた。

この部分には値を書くこともできて、SELECT文で`?1`などのパラメータにした値をここで指定することができる。  
`?1`や`?2`は順番通りに書かないといけないが、`:name`のように名前をつけておくと`stmt.query_map(&[(":name", "default"), (":id", "3")],...`のように書くことができる。
