...
ここで`dummy`も`\0`で終わっていないのに打ち切られたのはメモリの境目とかでゼロになっているだけじゃなかろうか。調べてない。  
コンパイラがwarningを出力しているのも`src`がコンパイル時にわかる内容だったからというだけで、動的なサイズであれば指摘できない。

このように、`dst`に書き込むサイズは`n`でとどめてくれるのだが、`dst`が`\0`で終わらないことになる。  
そういう関数仕様だから仕方ないのだが、まあ混乱しますわな。

だいたいの場合で期待する`strncpy`の使い方は、呼び出した後で`dst`の終わりに`\0`を書き込むことだろう。
