...
`OP_CHECKSEQUENCEVERIFY`(長いのでしばしば`OP_CSV`と略される)はトランザクションのバージョン`nVersion`が2以上でないと使用できない。

`OP_CSV`自体は、引数の数値とトランザクションの`nSequence`を比較して、`nSequence >= OP_CSV`であれば真となる。  
引数の数値はブロック数とタイムスタンプと2種類の表現があるが、まあ通常はブロック数を使うだろう。

これとは別に`nSequence`が関係するコンセンサスルールがあり、そのVINが指しているトランザクションのconfirmation数は`nSequence`以上でなくてはならないというものがある。
このルールと`OP_CSV`が合わさることで、前のトランザクションが`OP_CSV`で指定したconfirmation数以上経過しないと有効にできないというスクリプトを実現している。
コンセンサスルールがBIP-68なので、これにひっかかったトランザクションを展開するとBitcoin Coreは"non-BIP68-final"というようなメッセージを返す。
