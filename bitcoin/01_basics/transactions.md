...
楕円曲線の署名をトランザクションに載せることがある。
今のところ、ECDSA署名(P2TRより前)とSchnorr署名(P2TR)のどちらかである。  
どちらも楕円曲線の種類はsecp256k1だ。
...
DER形式の部分だけで 70～72 バイトと幅がある。署名のデータ長を加えると 71～73 バイトになるし、署名タイプを加えると 72～74 バイトになる。

#### Schnorr署名

P2TR というか、シュノア署名の場合はサイズが 64 バイト固定である。
署名対象が全体の場合は署名タイプを省略する(`SIGHASH_DEFAULT`)のでデータ長を加えても 65 バイト固定になる。

#### SIGHASH TYPE

「署名する」と書いているが、Bitcoinスクリプトで対処できるのはそのトランザクションに署名したものだけである。  
そのトランザクションのハッシュ値をとって、そのハッシュ値に秘密鍵で署名してトランザクションに載せ、その署名は公開鍵で検証する。

単独鍵の場合、P2TRより前は公開鍵がわからないのでそのトランザクションに一緒に載せて検証してもらう。
P2TRはアドレスがtweakedな公開鍵なのでそれをそのまま使って検証する(その分トランザクションが小さくなる)。  
スクリプトの場合はどの鍵で署名したかわからないので公開鍵はスクリプトに一緒に載せる。

そのハッシュの計算方法はいくつかある。
計算方法というよりは、トランザクションのどの部分を含めるかの選択ができる。  
SigHash Typeと呼ばれ、今のところ4つのタイプ値と1つのフラグがある1バイトの値である。  
`SIGHASH_DEFAULT`はP2TRになって登場したものなのでサイトによっては載っていないかもしれない。
...
通常はトランザクション全体が署名の対象範囲、つまりハッシュ値にする範囲を選択する。

* [btc: SINGLE + ANYONECANPAY - hiro99ma blog](https://blog.hirokuma.work/2026/05/20260524-btc.html)
