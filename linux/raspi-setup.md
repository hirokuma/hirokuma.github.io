...
2026/04/13にリリースされたRaspbianOS kernel v6.12の記事だが、`sudo` 実行にパスワードを必要にするよう変更したとのこと。

* [A security update for Raspberry Pi OS - Raspberry Pi](https://www.raspberrypi.com/news/a-security-update-for-raspberry-pi-os/)

普通に `apt update/upgrade` を済ませていたからか、再起動していないのにアップデートされていた。

```shell
$ uname -a
Linux uenosan 6.12.47+rpt-rpi-v8 #1 SMP PREEMPT Debian 1:6.12.47-1+rpt1~bookworm (2025-09-16) aarch64 GNU/Linux
```

実際に `sudo apt update` するとパスワードを求められた。
が、以前からそうだったかは記憶にない。。。
...
ちなみにデフォルトでは `sudo` を実行してから15分間はセッションが維持されるそうだ。
終わらせたければ `sudo -k` を実行すると良い。
